...
def evaluate_non_continuous_farm(user_params, potential_turbine_locs=None, algo_name=None, optimize_disc=False,
	...
	"""Evaluate initial profit of a discrete turbine farm with turbine locs chosen from the optimal
	...
	if optimize_disc:
		...
		output_dict.update(optimize_turbine_farm(f_farm, f_solver, f_controls, disc_functional,
		...
		output_dict.update(optimize_turbine_farm(b_farm, b_solver, b_control, b_functional,
												 {'cost': cost_per_turbine, 'iters': iternum, 'tolerance': tolerance},
												 'mipdeco'))
		...
		output_dict.update(evaluate_non_continuous_farm(user_params,
														binary_locs,
														algo_name='mipdeco_eval_as_discrete',
														init_two_step=True))
	return output_dict
